...
function cleanClasses(content) {
    ...
    content = content.replace(/<th([^>]*)>/g, (match, inner) => {
        ...
        return `<th className="font-semibold text-slate-600 dark:text-slate-300 px-3 py-2 text-center text-xs align-middle border-r border-slate-200 dark:border-slate-800 truncate" ${cleanInner.trim()}>`;
    });
    ...
    content = content.replace(/<td([^>]*)>/g, (match, inner) => {
        ...
        if (cleanInner.includes('Номенклатура') || inner.includes('nom') || inner.includes('text-left')) {
            align = 'text-left';
        }
    ...
    content = content.replace(/bg-([a-z]+)-(50|100|500\/10)/g, '');
    content = content.replace(/text-([a-z]+)-(400|500|600)/g, 'text-slate-800 dark:text-slate-200');
    content = content.replace(/border-([a-z]+)-(100|200|900\/40)/g, 'border-slate-200 dark:border-slate-800');
